	var aa=[
		...
		{ "name": "河北", "city":[
				 ...
				 {"name":"石家庄", 
				 "area":
				 ["长安区","桥东区","桥西区","新华区","郊  区","井陉矿区","井陉县","正定县","栾城县","行唐县","灵寿县","高邑县","深泽县","赞皇县","无极县","平山县","元氏县","赵  县","辛集市","藁","晋州市","新乐市","鹿泉市"]
				 },
		...
		var aaoption=aaobj.getElementsByTagName("option");
		var bboption=bbobj.getElementsByTagName("option");
		for(var i=0; i<aa.length;i++){
			aaobj.innerHTML+=("<option>"+aa[i]["name"]+"</option>");
			aaoption[i].value=aa[i]["name"];
		}
		aaobj.onblur=selects;
		function selects(){
			for(var i=0;i<aa.length;i++){
				if(this.value==aa[i]["name"]){
					for(var j=0;j<aa[i]["city"].length;j++){
						bbobj.innerHTML+=("<option>"+aa[i]["city"][j]["name"]+"</option>");
					}
				}
			}
		}
		aaobj.onfocus=selects2;
		function selects2(){
			bbobj.innerHTML="";
		}
